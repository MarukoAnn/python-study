...
- 就可以使用覆盖的方式，在子类重写编写 父类的方法实现

>具体的实现方式，就相当于在子类中定义一个和父类同名字的方法并实现

重写之后,在运行时，只会调用子类中重写的方法，而不再调用父类封装的方法
2) 对父类方法进行扩展
- 如果在开发中，子类的方法实现 中 包含 父类的方法实现
  - 父类原本封装了的方法实现 时子类方法的一部分
- 就可以使用 扩展的方式     
  1、在子类中 重写 父类的 方法     
...
父类名.方法(self)
```
- 这种方式，目前在Python3.x还支持
- 这种方式不推荐使用，因为一旦父类发生改变，方法调用位置的类名同样需要修改

提示
- 在开发时，父类名 和 super() 两种方式不要混用
- 如果使用 当前子类名调用方法，会形成递归调用，出现死循环

######7.1.3 父类的私有属性和私有方法
1、子类对象不能在自己的方法内部，直接访问父类的私有属性或私有方法
2、子类对象可以同过父类的公有方法间接访问到私有属性或私有方法
> 1、私有属性、方法是对像的隐私，不对外公开，外界以及子类都不能直接抽象访问
...
> 提示：开发时，应尽量避免这中容易产生混淆的情况！--如果父类之间存在同名的属性或者方法，
应尽量避免使用多继承

Python中的MRO --方法搜索顺序(知道)
- python 中针对 类提供给了一个内置属性 __mro__ 可以查看方法 搜索顺序
- MRO 是 method resilution order， 主要用于多继承时判断方法、属性、路径
...
```
- 在搜索方法时，是按照 __mro__的输出结果从左至右的顺序查找的
- 如果在当前类中找到方法，就直接执行，不在搜索
- 如果没有找到，就查找下一个类中是否有对应的方法，如果找到，就直接执行，不在搜索
- 如果找到最后一个类，都还没有找到方法，程序报错

###### 7.2.1 新式类与旧式(经典)类
> object 是Python 为所有对象提供的基类，提供一些内置的属性和方法，可以使用dir函数查看

- 新式类：以object为基类的类，推荐使用
- 经典类：不以object为基类的类，不推荐使用

> 新式类和旧式类 在多继承时 -- 会影响到方法的搜索顺序

为了保证编写的代码能够同时在Python2.x 和 Python3.x 运行！
今后定义类时，如果没有父类，建议同意继承自object
```
class 类名(object):
    pass
```
...
##### 多态
面向对象三大特性
1、***封装*** 根据职责将属性和方法封装到一个抽象得类中
2、***继承*** 实现代码的重用，相同的代码不需要重复的编写
3、***多态***  不同的子类对象调用父类方法，产生的不同的执行结果
- 多态 可以增加代码的灵活度
- 以继承的 和 重写父类的方法 为前提
...
- 类方法第一个参数应该是cls
    ...
    - 这个参数和实例方法的第一个参数是self参数类似
    - 提示使用其他名称也可以，调用方法时，不需要传递cls参数
- 在方法内部
    - 可以通过cls.访问类的属性
    - 也可以通过cls.调用其他类方法

> 在类方法内部，可以直接使用cls访问类属性 或者调用类方法

##### 静态方法
- 在开发时间，如果需要在类 中封装一个方法，这个方法:
    - 既不需要访问实列属性 或者调用实例方法
    - 也不需要访问类属性 或者调用类方法
...
def 静态方法名():
    pass
```
- 静态方法需要使用装饰器@staticmethod 来标识，告诉解释器这是一个静态方法
- 通过类名. 调用静态方法
...
###### 01 异常的概念
- 程序在运行时，如果Python 解释器遇到一个错误，会停止程序的执行，并且提示一些错误信息，这就是异常
- 程序停止执行并且提示错误信息，这个动作，我们通常称之为 抛出(ralse)异常

> 程序开发时，很难将所有的特殊情况 处理的面面俱到，通过异常捕获可以针对突发事件做集中的处理，
 从而保证程序的稳定性和健壮性
...
- exept 如果不是，下方编写的代码尝试失败的代码

2.2 错误类性捕获
- 在程序执行时, 可能会遇到不同的类型异常，并且需要针对不同的类型的异常，做出不同的响应，这个时候就需要捕获错误类型了
...
try: 
    ...
    pass
expect (错误类型2, 错误类型3):
    # 针对错误类型2，错误类型3对应的代码处理
    pass
expect Exception as result:
    print("未知错误 _%s" % result)
```
- 当Python 解释器 抛出异常时，最后一行错误信息的第一个单词，就是错误类型

**捕获未知错误**
- 在开发时，要预判到所有可能出现的错误，还是有一定难度的
- 如果希望程序无论出现任何错误，都不会因为Python 解释器抛出异常而被终止，可以增加一个except
...
2.3 异常捕获的完整语法
- 在实际开发中，为了能够处理复杂的情况，完整的异常语法如下:
> 提示：
> - 有关完整的语法应用场景，在后续学习中，结合实际的案例会更好理解
> - 现在先对这个语法结构有个印象即可
```
try: 
    # 尝试执行的代码
...
```
- else 只有没有异常时才会执行的代码
- finally 无论是否有异常，都会执的代码

###### 03 异常的传递
- 异常的传递 -- 当 函数/方法 执行出现异常，会将异常传递给函数/方法 的调用一方
- 如果传递到主程序，仍然没有异常处理，程序还是会被终止

> 提示

- 在开发中，可以在主函数中增加 异常捕获
- 而在主函数中调用其他函数，只需要出现异常，都会传递到主函数的异常捕获中
